...
BOOL CAlarmsListDialog::OnInitDialog() 
{
        ...
        l_nCpt++)
    {
        ...
        l_strText.Format("%-2d %s,%d %d:%d", 
            l_pAlarmNode->m_timeRing.GetDay(),
            m_pView->GetMonthName(l_pAlarmNode->m_timeRing.
                    GetMonth()),
            l_pAlarmNode->m_timeRing.GetYear(),
            l_pAlarmNode->m_timeRing.GetHour(),
            l_pAlarmNode->m_timeRing.GetMinute());
...
CString CAlarmsListDialog::TimeString(CTime & l_time)
{
